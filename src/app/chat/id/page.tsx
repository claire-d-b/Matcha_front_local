          ...
          <div className="md:w-1/2 self-center bg-white rounded-lg p-12 shadow-lg overflow-scroll">
            {messages.length &&
              messages.map((m, i) => (
                <div
                  key={`message_${i}`}
                  className="flex flex-col gap-1 mb-8 md:mb-4 text-sm text-white justify-center items-center"
                >
                  <div
                    className={
                      m.mine
                        ? "mb-2 md:mb-4 text-gray-800 self-end"
                        : "mb-2 md:mb-4 text-gray-800 self-start"
                    }
                  >
                  ...
                  <Paper
                    ...
                    className={
                      m.mine
                        ...
                        : "py-1 px-2 md:px-4 md:py-2 self-start rounded-lg font-semibold text-white"
                    }
                  >
                    {m.mine ? "You" : m.author}
                  </Paper>
                  <div
                    className={
                      m.mine
                        ? "py-1 px-2 md:px-4 md:py-2 self-end bg-gray-400 rounded-lg font-thin"
                        : "py-1 px-2 md:px-4 md:py-2 self-start bg-gray-400 rounded-lg font-thin"
                    }
                  >
                    {m.content}
                  </div>
                  {(m.received && (
                    <IconButton
                      ...
                      className={
                        m.mine
                          ? "mt-0 self-end bg-transparent"
                          : "mt-0 self-start bg-transparent"
                      }
                      disabled
                    >
                      <CheckCircleIcon
                        className={m.mine ? "text-sm" : "text-sm text-gray-800"}
                      />
                    </IconButton>
                  )) || (
                    <IconButton
