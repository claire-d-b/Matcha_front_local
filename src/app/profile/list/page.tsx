...
const _ = () => {
    ...
    const handleFilter = (event: React.ChangeEvent<HTMLInputElement>, i: number) => {
        const nfilter = [...filter]
        if (event.target.checked && filter.indexOf(values[i]) === -1)
            nfilter.push(values[i])
        else
            nfilter.splice(filter.indexOf(values[i]),1)
        setFilter(nfilter)
        console.log(nfilter)
    }
    ...
    return (
        ...
        <div className="flex flex-col h-full w-screen bg-gray-300">
